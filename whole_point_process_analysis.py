...
def maps_and_rates(in_file, seed_location_ecn, seed_location_dna, seed_location_dnb, outdir, sub, map_ecn_file_name = 'map_ecn', map_dna_file_name = 'map_dna', map_dnb_file_name = 'map_dnb'):
    ...
    total_ecn_activations = np.count_nonzero(pp_seed_data_ecn[:-1])
    
    rate_ecn_dna = np.count_nonzero((pp_seed_data_ecn[:-1] + pp_seed_data_dna[1:]) == 2) / total_ecn_activations if total_ecn_activations > 0 else 0.0
    rate_ecn_dnb = np.count_nonzero((pp_seed_data_ecn[:-1] + pp_seed_data_dnb[1:]) == 2) / total_ecn_activations if total_ecn_activations > 0 else 0.0
